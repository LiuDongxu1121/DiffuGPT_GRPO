...
def evaluate_equation(equation_str):
    ...
    if not parts:
        return None
    last_eq = parts[-1].split('=')
    if len(last_eq) != 2:
        return None
    try:
        return int(last_eq[1].strip())
    except ValueError:
        return None
...
def compute_score(solution_str, ground_truth, method="strict", format_score=0.1, score=1.0):
    ...
    if do_print:
        ...
        #print(f'completions: {solution_str}')
    ...
    if not validate_equation(equation):
        if do_print:
            print(f"Invalid equation score: {format_score}")
        return format_score
    ...
    try:
        ...
        if abs(result - target) < 1e-5:
            if do_print:
                print(f"Pred result: {target}")
                print(f"Correct result score: {score}")
            return score
        else:
            if do_print:
                print(f"Pred result: {target}")
                print(f"Wrong result score: {score}")
            return format_score
    except:
        if do_print:
